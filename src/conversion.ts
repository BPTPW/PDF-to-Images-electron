...
export function resolvePdftocairoPath(): string {
  ...
  return process.env.PDFTOCAIRO_PATH || join(root, 'resources', 'poppler', platformDirectory, executable);
}
...
async function runPdftocairo(inputPath: string, prefix: string, dpi: number, format: ConversionOptions['format']): Promise<void> {
  const executable = resolvePdftocairoPath();
  await new Promise<void>((resolve, reject) => {
    const child = spawn(executable, [`-${popplerFormat(format)}`, '-r', String(dpi), inputPath, prefix], { windowsHide: true });
    let stderr = '';
    child.stderr.on('data', (data: Buffer) => { stderr += data.toString(); });
    child.on('error', (error) => {
      reject(new Error(`无法启动 PDF 渲染器（${executable}）：${error.message}`));
    });
    child.on('close', (code) => {
      if (code === 0) resolve();
      else reject(new Error(stderr.trim() || `PDF 渲染失败，退出代码 ${code}`));
    });
  });
}
...
export async function convertPdf(options: ConversionOptions): Promise<{ outputDirectory: string; pageCount: number }> {
  ...
  try {
    await runPdftocairo(options.inputPath, prefix, options.dpi, options.format);
    ...
    if (pages.length === 0) throw new Error('未生成任何页面，请确认 PDF 文件有效且未加密。');

    await mkdir(outputDirectory, { recursive: true });
    const width = pageNumberWidth(pages.length);
    for (let index = 0; index < pages.length; index += 1) {
      const number = String(index + 1).padStart(width, '0');
      await rename(join(temporaryDirectory, pages[index]), join(outputDirectory, `${number}.${options.format}`));
    }
