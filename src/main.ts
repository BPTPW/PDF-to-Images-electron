...
app.whenReady().then(() => {
  ...
  ipcMain.handle('pdf:convert', async (_event, options: ConversionOptions) => {
    if (!options || !['png', 'jpg', 'tiff'].includes(options.format) || !Number.isFinite(options.dpi)) {
      throw new Error('转换参数无效。');
    }
    return convertPdf(options);
  });

  ipcMain.handle('pdf:output-directory', (_event, inputPath: string) => outputDirectoryFor(inputPath));
  createWindow();
