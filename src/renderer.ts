...
convertButton.addEventListener('click', async () => {
    ...
    convertButton.innerHTML =
        '<i class="layui-icon layui-icon-loading layui-anim layui-anim-rotate layui-anim-loop"></i>正在转换';
    ...
    try {
        ...
        showStatus(
            `转换完成：已导出 ${result.pageCount} 页到 ${result.outputDirectory}`,
            'success',
        );
    } catch (error) {
        showStatus(
        ...
        convertButton.disabled = false;
        convertButton.innerHTML =
            '<i class="layui-icon layui-icon-release"></i>开始转换';
    }
});
